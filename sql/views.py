...
def submitSql(request):
    ...
    to_person = users.objects.all().exclude(username=loginUser)
    listAllToPerson = [user.email for user in to_person]

    context = {'currentMenu': 'submitsql', 'dictAllClusterDb': dictAllClusterDb, 'reviewMen': reviewMen, 'to_person':listAllToPerson}
...
def autoreview(request):
    ...
    toPerson = request.POST.getlist('listAllToPerson')
    ...
    if workflowStatus == Const.workflowStatus['manreviewing']:
        ...
        if hasattr(settings, 'MAIL_ON_OFF') == True:
            if getattr(settings, 'MAIL_ON_OFF') == "on":
                ...
                # 发一封邮件
                strTitle = "新的SQL上线工单提醒#" + str(workflowId)+",请审核"
